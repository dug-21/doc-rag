...
use super::super::classification::{
    document_classifier::{DocumentClassifier, DocumentTypeResult, SectionTypeResult, QueryRoutingResult, DocumentType},
};
...
impl SmartIngestionPipeline {
    ...
    pub async fn process_document(&mut self, content: &str, metadata: Option<HashMap<String, String>>) -> Result<ProcessedDocument> {
        ...
        let max_processing_time_ms = self.config.max_processing_time_ms as f64;
        ...
        if total_processing_time_ms > max_processing_time_ms {
            warn!("Document processing exceeded time limit: {:.1}ms > {}ms",
                  total_processing_time_ms, max_processing_time_ms);
        }

        info!("Document {} processed successfully in {:.1}ms",
              document_id, total_processing_time_ms);
...
mod tests {
    ...
    async fn test_query_routing() {
        ...
        assert!(matches!(result.routing_decision, crate::ingestion::classification::document_classifier::QueryRoute::Symbolic));
